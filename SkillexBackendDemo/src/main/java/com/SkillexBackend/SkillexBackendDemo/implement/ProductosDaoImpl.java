...
public class ProductosDaoImpl implements ProductosDao {
	...
	public List<ProductosVO> mostrar_productos() {
		...
		try {
			...
			String sql = "SELECT p.id_productos," + "p.nombre_producto," + "p.nombre_producto_in,"
					+ "p.codigo_producto," + "p.estado_producto," + "p.cantidad_producto," + "p.fecha_ingreso,"
					+ "dp.id_detalle_productos," + "dp.valor_inicial," + "dp.valor_mas_iva,"
					+ "dp.descripcion_producto," + "dp.descripcion_producto_in," + "dp.nombre_imagen,"
					+ "dp.url_imagen," + "cp.id_categoria_producto," + "cp.tipo_categoria " + "FROM productos p "
			...
			if (listOb.size() > 0) {
				...
				while (it.hasNext()) {
					...
					pro = new ProductosVO(id_productos, nombre_producto, nombre_producto_in, codigo_producto,
							estado_producto, cantidad_producto, fecha_ingreso, id_detalle_productos, valor_inicial,
							valor_mas_iva, descripcion_producto, descripcion_producto_in, nombre_imagen, url_imagen,
							id_categoria_producto, tipo_categoria, null, "", "");
	...
	public Object save(ProductosCrearVO producto) {
		...
		producto.setUrl_imagen(
				this.imagenCreate.convertirImagen(producto.getUrl_imagen(), producto.getNombre_imagen(), 1));
		...
		if (id_Productos == 0) {
			try {
				...
				query.setParameter("categoria_producto_id_categoria_producto",
						producto.getCategoria_producto_id_categoria_producto());
	...
	public Object updateProducto(ProductosVO producto) {
		...
		if (proUp.getIdProductos() != null) {
			...
			try {
				// Update a productos
				String sql = "update productos " + "set nombre_producto = :nombre, " + "cantidad_producto = :cantidad, "
						+ "categoria_producto_id_categoria_producto = :categoria " + "where id_productos = :idPro";
				...
				String sql2 = "Update detalle_productos " + "set descripcion_producto = :descripcion, "
						+ "valor_inicial = :valorInicial, " + "valor_mas_iva = :valorIva, "
						+ "nombre_imagen = :nombreImagen," + "url_imagen = :url "
	...
	public Object reporte(String format) throws FileNotFoundException, JRException {
		...
		ReporteVO reporteBase64 = new ReporteVO(pdfBase64, "productosReportes");

...
import com.SkillexBackend.SkillexBackendDemo.controllers.Consultas;
...
import com.SkillexBackend.SkillexBackendDemo.vo.ProUpdateVO;
 ...
 */
@Service
public class PedidosDaoImpl extends Consultas implements PedidosDao {
	...
	public Object create(PedidosVO pedido) {
		...
		try {
			...
			// Integer lastDetalle = metodos.contsultarUltimoID("detallepedido",
			// "iddetallePedido", "valorApagar");
			// Consultar ultimo Id
			...
			auxiliar2.close();
			// Tercer insert
			// Integer lastPedido = (Integer) metodos.contsultarUltimoID("pedidos",
			// "idPedidos", "mesa");
	...
	public List<PedidosVO> getPedidos() {
		...
		try {
			String sql = "SELECT " + "p.idPedidos," + "p.estadoPedido," + "p.mesa," + "dp.iddetallePedido,"
					+ "dp.valorApagar," + "dp.producto," + "dp.cantidadProducto," + "dp.fecha_pedido,"
					+ "ep.id_Estado_pedido," + "ep.State_id," + "ep.descripcion," + "php.cantidad_producto_pedido,"
					+ "pr.id_productos," + "pr.nombre_producto," + "pr.codigo_producto," + "pr.estado_producto,"
					+ "pr.cantidad_producto," + "pr.fecha_ingreso," + "dpr.id_detalle_productos," + "dpr.valor_inicial,"
					+ "dpr.valor_mas_iva," + "dpr.descripcion_producto," + "dpr.nombre_imagen," + "dpr.url_imagen,"
					+ "u.idUsuarios," + "u.nombreUsuario," + "u.apellidoUsuario," + "u.emailUsuario,"
					+ "u.passwordUsuario," + "u.tienda," + "u.creacion_usuario," + "u.fecha_login,"
					+ "u.turnos_laborales," + "u.cedula_ciudadania " + "FROM skillexbd.pedidos p "
			...
			while (it.hasNext()) {
				...
				almacenarPedidos = new ListarPedidosVO(idPedidos, estadoPedido, mesaP, iddetallePedido, valorApagar,
						producto, cantidadProducto, fecha_pedido, Id_Estado_pedido, State_id, descripcion,
						cantidad_producto_pedido, id_productos, nombre_producto, codigo_producto, estado_producto,
						cantidad_producto, fecha_ingreso, id_detalle_productos, valor_inicial, valor_mas_iva,
						descripcion_producto, nombre_imagen, url_imagen, idUsuarios, nombreUsuario, apellidoUsuario,
						emailUsuario, passwordUsuario, tienda, creacion_usuario, fecha_login, turnos_laborales,
						cedula_ciudadania);
				...
				for (ListarPedidosVO itemTraidos : LPVO) {
					if (listadoPedidos.size() > 0) {
						...
						if (idPedido != null) {
							for (PedidosVO itemP : listadoPedidos) {
								if (itemP.getIdPedidos() == idPedido) {
									productos = new ProductosVO(itemTraidos.getId_productos(),
											itemTraidos.getNombre_producto(), "", itemTraidos.getCodigo_producto(),
											itemTraidos.getEstado_producto(), itemTraidos.getCantidad_producto_pedido(),
											itemTraidos.getFecha_ingreso(), itemTraidos.getId_detalle_productos(),
											itemTraidos.getValor_inicial(), itemTraidos.getValor_mas_iva(),
											itemTraidos.getDescripcion_producto(), "", itemTraidos.getNombre_imagen(),
											itemTraidos.getUrl_imagen(), null, "", null, "", "");
							...
							pedido = new PedidosVO(itemTraidos.getIdPedidos(), itemTraidos.getEstadoPedido(),
									itemTraidos.getMesa(), itemTraidos.getIddetallePedido(),
									itemTraidos.getIdUsuarios(), itemTraidos.getId_Estado_pedido(),
									itemTraidos.getValorApagar(), itemTraidos.getFecha_pedido(), null, null);
							usuario = new UsuarioVO(itemTraidos.getIdUsuarios(), itemTraidos.getNombreUsuario(),
									itemTraidos.getApellidoUsuario(), itemTraidos.getEmailUsuario(), "",
									itemTraidos.getTienda(), itemTraidos.getCreacion_usuario(),
									itemTraidos.getFecha_login(), itemTraidos.getTurnos_laborales(),
									itemTraidos.getCedula_ciudadania(), null, null);
							pedido.setUsuario(usuario);
							productos = new ProductosVO(itemTraidos.getId_productos(), itemTraidos.getNombre_producto(),
									"", itemTraidos.getCodigo_producto(), itemTraidos.getEstado_producto(),
									itemTraidos.getCantidad_producto_pedido(), itemTraidos.getFecha_ingreso(),
									itemTraidos.getId_detalle_productos(), itemTraidos.getValor_inicial(),
									itemTraidos.getValor_mas_iva(), itemTraidos.getDescripcion_producto(), "",
									itemTraidos.getNombre_imagen(), itemTraidos.getUrl_imagen(), null, "", null, "",
									"");
						...
						pedido = new PedidosVO(itemTraidos.getIdPedidos(), itemTraidos.getEstadoPedido(),
								itemTraidos.getMesa(), itemTraidos.getIddetallePedido(), itemTraidos.getIdUsuarios(),
								itemTraidos.getId_Estado_pedido(), itemTraidos.getValorApagar(),
								itemTraidos.getFecha_pedido(), null, null);
						usuario = new UsuarioVO(itemTraidos.getIdUsuarios(), itemTraidos.getNombreUsuario(),
								itemTraidos.getApellidoUsuario(), itemTraidos.getEmailUsuario(), "",
								itemTraidos.getTienda(), itemTraidos.getCreacion_usuario(),
								itemTraidos.getFecha_login(), itemTraidos.getTurnos_laborales(),
	...
	public Object procesar(PedidosVO pedidos) {
		ProUpdateVO producAc = new ProUpdateVO();
		List<ProUpdateVO> listProAc = new ArrayList<ProUpdateVO>();
		emf = Persistence.createEntityManagerFactory("com.miUnidadDePersistencia");
		EntityManager em = emf.createEntityManager();
		em.getTransaction().begin();
		RespuestaOperaciones resp = new RespuestaOperaciones();
		...
		try {
			...
			if (pedidos.getIdEstadoPedido() == 1) {
				EntityManager em3 = emf.createEntityManager();
				em3.getTransaction().begin();
				String sql3 = "SELECT id_productos,cantidad_producto FROM productos WHERE id_productos IN (";
				ProductosVO[] item = pedidos.getProducto().toArray(new ProductosVO[0]);
				for (Integer i = 0; i < pedidos.getProducto().size(); i++) {
					if (i == (pedidos.getProducto().size() - 1)) {
						sql3 = sql3 +  item[i].getIdProductos() + ")";
					} else {
						sql3 = sql3 +  item[i].getIdProductos() + ",";
					}
				}
				...
				List<Object[]> listaResultados = query3.getResultList();
				...
				if (!listaResultados.isEmpty()) {
					...
					while(it.hasNext()) {
						...
						producAc.setId((Integer) line[0]);
						producAc.setCantidad((Integer) line[1]);
						
						listProAc.add(producAc);
						producAc = new ProUpdateVO();
					}
					
					if (!listProAc.isEmpty()) {
						EntityManager em4 = emf.createEntityManager();
						em4.getTransaction().begin();
						...
						for (ProductosVO producto : pedidos.getProducto() ) {
							String consulta2 = "";
							for (ProUpdateVO proCon : listProAc) {
								if (producto.getIdProductos() == proCon.getId()) {
									String cantidadFinal = String.valueOf(proCon.getCantidad() - producto.getCantidadProducto());
									String idUpdate = String.valueOf(producto.getIdProductos());
